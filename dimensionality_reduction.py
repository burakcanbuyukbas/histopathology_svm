...
def concat_data(path="data_batches/scaled/X_train", j=20, target="data/pca/X_train.npy"):
    arrays = []
    for i in tqdm(range(j)):
        X_add = np.load(path + str(i) + ".npy")
        arrays.append(X_add)
        print(X_add.shape)
...
def normalized_pca(data, batch=1000, components=None):
    k = 0
    n = data.shape[0]  # total size
    chunk_size = batch  # batch size
    ipca = IncrementalPCA(n_components=components, batch_size=batch)
...
normalized_pca(data, batch=5000, components=None)
# concat_data(path=)
